                   ...
                   const CInuError &iError)
{
    ...
    // TODO 相机点云输出方向相反，需要手动翻转一下
    pcl::transformPointCloud(*cloud, *cloud, rotation_correct);
    std::cout << "PointCloud before filtering has: " << cloud->points.size() << " data points." << std::endl;
    std::cout << "PointCloud Width: " << cloud->width << "\nPointCloud Height: " << cloud->height << std::endl;

    TicToc t_vg;
    pcl::VoxelGrid<pcl::PointXYZ> vg;
    vg.setInputCloud(cloud);
    vg.setLeafSize(0.015f, 0.015f, 0.015f); // unit: m
    vg.filter(*cloud_filtered);
    std::cout << "PointCloud after VoxelGrid has: " << cloud_filtered->points.size() << " data points." << std::endl;
    std::cout << "VoxelGrid time cost:" << t_vg.toc() << " ms" << std::endl;
    ...
    // std::cout << "PointCloud after SACSegmentation has: " << cloud_filtered->points.size() << " data points." << std::endl;
    ...
    std::cout << "PointCloud for DBSCANKdtreeCluster has: " << cloud_filtered->points.size() << " data points." << std::endl;
         ...
         it != cluster_indices.end(); it++, j++)
    {
        ...
        cv::rectangle(rgb, p_min, p_max, Scalar(255, 0, 0), 2, cv::LINE_8);
        obstacle_viewer->addCube(min_point_AABB.x, max_point_AABB.x, min_point_AABB.y, max_point_AABB.y, min_point_AABB.z, max_point_AABB.z, 1.0, 1.0, 1.0, aabb_id);
        obstacle_viewer->setShapeRenderingProperties(pcl::visualization::PCL_VISUALIZER_REPRESENTATION_WIREFRAME, 1.0, aabb_id);
